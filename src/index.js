...
`;
            ...
            try {
                ...
                console.log(`✅ Script cached successfully (${script.length} bytes)`);
...
app.get('/api/admin/stats', adminAuth, (req, res) => {
    res.json({ 
        ...
        stats: db.getStats(),
        config: {
            scriptSourceConfigured: !!config.SCRIPT_SOURCE_URL,
            scriptSourceUrl: config.SCRIPT_SOURCE_URL ? '***configured***' : 'NOT SET'
        },
        cache: {
...
app.use('*', (req, res) => {
    ...
    const notFoundScript = `warn("[LOADER] Invalid endpoint: ${req.originalUrl}")`;
    res.status(200).type('text/plain').send(notFoundScript);
});
...
app.listen(PORT, '0.0.0.0', () => {
    ...
    if (config.SCRIPT_SOURCE_URL) {
        ...
        console.log('║  ❌ SCRIPT_SOURCE_URL: NOT SET! Set it in Render ENV     ║');
    }
    
    if (config.ADMIN_KEY && config.ADMIN_KEY !== 'change-this-to-secure-key') {
